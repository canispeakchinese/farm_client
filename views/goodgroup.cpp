...
QRectF GoodItem::boundingRect() const//我猜是用来确定刷新区域的，但是不确定
{
    if(business == Use)
...
QPainterPath GoodItem::shape() const//这个函数返回值确定的是这个item的作用范围，比如点击事件,碰撞检测
{
    QPainterPath path;
    if(business == Use)
...
{
    if(!levelEnough && business != Sell)//设定当前等级不能买的物品可以卖也可以用,为什么可以用呢，因为嫌麻烦，以后再改吧
    {
...
QRectF GoodGroup::boundingRect() const//我猜是用来确定刷新区域的，但是不确定
{
    if(business == Use)
...
QPainterPath GoodGroup::shape() const//这个函数返回值确定的是这个item的作用范围，比如点击事件,碰撞检测
{
    QPainterPath path;
    if(business == Use)
    {
...
void GoodGroup::startBusiness(Good good)
{
    if(business == Buy || business == Sell)
    {
        ...
        connect(businessdialog,SIGNAL(business_success(Good,int)),this,SLOT(getInfor(Good,int)));
        businessdialog->exec();
    }
    else
        getInfor(good, 1);
...
void GoodGroup::getInfor(Good good, int businessNum)
{
    emit sendRequest(good, businessNum);
}
